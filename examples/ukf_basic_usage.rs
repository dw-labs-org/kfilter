...
use nalgebra::{Matrix2, Vector2, SMatrix, SVector};
use kfilter::ukf::*;
...
fn basic_2d_tracking() {
    ...
    for step in 0..10 {
        ...
        println!("Step {}: pos={:.3}, vel={:.3}, uncertainty={:.3}", 
            step, ukf.state()[0], ukf.state()[1], ukf.covariance().diagonal().norm());
    }
}
...
fn nonlinear_measurement_example() {
    ...
    for step in 0..5 {
        ...
        println!("Step {}: pos=({:.3}, {:.3}), estimated_range={:.3}", 
            step, ukf.state()[0], ukf.state()[1], current_range);
    }
}
...
fn input_system_example() {
    ...
    for step in 0..20 {
        ...
        if step % 5 == 0 {
            println!("Step {}: pos={:.3}, vel={:.3}, control={:.3}", 
                step, ukf.state()[0], ukf.state()[1], control_input[0]);
        }
    }
    
    println!("Final position: {:.3} (target: {:.3})", ukf.state()[0], target_position);
}
...
fn parameter_comparison_example() {
    ...
    for &alpha in &alphas {
        ...
        println!("Alpha {:.3}: final_state=[{:.6}, {:.6}]", 
            alpha, ukf.state()[0], ukf.state()[1]);
    }
}